...
public class ShardInfoFactory {
	...
	public static List<JedisGroupShardInfo> createJedisGroupShardInfos(String masterSlaveUrlInfo, int socketTimeout, int connectionTimeout){
		try {
			/*
			...
			for(String msUrlInfo : msUrlInfoArray){
				...
				if(firstIndex > 0){
					...
					masterInfo.setSoTimeout(socketTimeout);
					masterInfo.setConnectionTimeout(connectionTimeout);
					...
					if(surls.length > 0){
						...
						for(String surl : surls){
							...
							slaveInfo.setSoTimeout(socketTimeout);
							slaveInfo.setConnectionTimeout(connectionTimeout);
	...
	public static List<JedisGroupShardInfo> createJedisGroupShardInfos(String masterSlaveUrlInfo){
		return createJedisGroupShardInfos(masterSlaveUrlInfo, 0, 0);
	}
