...
public interface RedisClient {
	 ...
	 * 		获取回源客户端。
	 */
	LoadingBackClient getLoadingBackClient();
	...
	/**
	 * 给指定键增对应的哈西表中的给定字段加对应的数值。
	 * <p>如果不存在key对应的值，那么会先将key的值设置为0，
	 * 然后执行hincrBy操作，返回increment。
	 *
	 * @param key 哈希表在redis中的key。
	 * @param field 哈希表中的字段。
	 * @param increment 增量值。
	 ...
	 * @throws RedisClientRuntimeException 如果发生异常等。
	 */
	long srem(final String key,final String... values);
	 ...
	 *      Redis集群统计信息。
	 */
	RedisClusterStatistic getStatistic();
